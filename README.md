...
question: *"what did this appliance cost me, in money?"* Appliance Energy Cost
turns existing per-appliance cumulative energy sensors and one all-inclusive
dynamic price sensor into trustworthy money figures: a live cost sensor that
accrues at the price in force at the moment of consumption, and a one-time,
explicitly confirmed backfill that reconstructs past cost from the hourly
history Home Assistant already stores. Everything is local and
Home-Assistant-native — no cloud, no accounts, no telemetry — and deliberately
conservative with the statistics database: a restart, a price outage, or an
import must never silently corrupt the numbers.

## What it does — and what it does not do

It does:

- Give every configured appliance a cumulative money-cost sensor
  (`device_class: monetary`, `state_class: total`). Energy consumed during one
  price period is settled at that period's price — never retro-priced at a
  later price, live or in backfill.
- Reconstruct historical cost into long-term statistics from the hourly
  history Home Assistant already stores — preview first, explicit confirm
  required, overlap protection on by default.
- Convert units (Wh/kWh/MWh, and price per kWh vs per MWh) — nothing more. The
  all-inclusive price sensor owns fees, taxes, and margins; this integration
  never adds price components of its own and offers no tariff how-tos.

It does not:

- **Estimate power.** That is
  [powercalc](https://github.com/bramstroker/homeassistant-powercalc)'s job —
  only energy an existing sensor has already measured is costed.
- **Model tariffs or billing.** No standing charges, VAT fields, tariff
  blending, or contract modelling — the same exclusion
  [dynamic_energy_cost](https://github.com/martinarva/dynamic_energy_cost)
  makes.
- **Administer the statistics database.** It writes only through the supported
  statistics import API, touches only its own statistic IDs, and has no
  deletion path.
- **Replace the Energy Dashboard.** The cost sensors render through standard
  Home Assistant cards.

What sets it apart is the combination: live cost priced at the moment of
consumption, plus a preview-first backfill of the same series backwards in
time into long-term statistics.
...
- [Configuration](#configuration)
  - [Price sensor and currency](#price-sensor-and-currency)
  - [Appliances](#appliances)
  - [Reconfiguring](#reconfiguring)
  - [Configuration errors](#configuration-errors)
- [The cost sensor](#the-cost-sensor)
...
- [Backfilling history](#backfilling-history)
  ...
  - [calibrate_cost](#calibrate_cost)
- [Cutover: joining live to imported history](#cutover-joining-live-to-imported-history)
  - [Standard cutover](#standard-cutover)
  - [Retro-fix: the sensor ran live before the import](#retro-fix-the-sensor-ran-live-before-the-import)
  - [Verifying the boundary](#verifying-the-boundary)
...
The repository is not in the HACS default store; add it as a
[custom repository](https://hacs.xyz/docs/faq/custom_repositories/):

1. In HACS, open **Custom repositories** and add
   `https://github.com/jonikanerva/appliance_energy_cost` with type
   **Integration**.
2. Download **Appliance Energy Cost** in HACS and restart Home Assistant.
3. Go to **Settings → Devices & services → Add integration** and search for
   **Appliance Energy Cost**. It is a config entry, not a helper — it does not
   appear in the Helpers panel.
...
Daily and monthly figures are built with Home Assistant's own
...
> your real IDs; the IDs in every example in this README are illustrative.
...
1. Remove individual appliances by deleting them from the integration's entry
   page; each appliance's cost sensor is removed with it.
2. Remove the config entry (or the whole integration) from
   **Settings → Devices & services**, then uninstall the repository in HACS
   and restart Home Assistant.

Long-term statistics are not deleted with the integration: removing the
integration does not delete imported rows, and the cost sensors'
live-recorded statistics remain as well. The orphaned series can be inspected
